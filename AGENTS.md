# AGENTS Context for AI Assistants
...
## Target OS: Debian, RedHat, and Archlinux
...
| `system_discovery_watermark_scale_factor` | `clamp(ceil(500*8192/RAM_MB), 10, 1000)` | kernel_tuning | Keeps absolute reclaim zone ~constant |
| `system_discovery_earlyoom_min_free_pct` | `clamp(ceil(40000/RAM_MB), 3, 10)` | oom_handler | Targets ~400MB absolute OOM trigger |
...
## No `|| true` or `|| echo` in Makefile Targets
...
The playbook assumes passwordless sudo (NOPASSWD in /etc/sudoers). Asking for a sudo password in an automated playbook breaks unattended runs, requires a terminal, and cannot be scripted. If passwordless sudo is not configured, the playbook fails immediately rather than prompting and then failing on the next task.

## Variable Names Prefixed With Role Name (ansible-lint rule)
...
## Cross-role Variables Prefixed With system_discovery

Facts set by system_discovery and consumed by other roles are prefixed with the source role name, not the consumer role name. This makes data provenance explicit: when reading `system_discovery_target_zram_mb` in zram_swap, the origin is clear. Renaming per-role would obscure the origin and require copying variables across roles.

## `changed_when: false` on Read-Only Tasks
...
## `changed_when: true` on State-Modifying shell/command Tasks
...
## MGLRU Value Checked Before Writing

Writing to `/sys/kernel/mm/lru_gen/enabled` triggers MMU notifiers. Without the check, Ansible reports "changed" every run because the shell module's output differs (the write always succeeds). Reading the current value and comparing avoids unnecessary writes and makes dry-run output truthful.

## GRUB Regex Uses `\bpsi=1\b`
...
## ansible_facts['*'] Instead of ansible_* Variables
...
Comments rot. They drift from implementation, excuse unclear code, and add visual noise. Every piece of code must be self-documenting: function names, variable names, and structure must convey intent without inline annotations. This applies to every file: YAML tasks, Python scripts, shell snippets, Makefile recipes. A task name in Ansible is not a comment; it is structured metadata displayed during execution and is permitted. A `#` comment or a `"""docstring"""` in Python is forbidden.

## No Decorative Symbols in Output
